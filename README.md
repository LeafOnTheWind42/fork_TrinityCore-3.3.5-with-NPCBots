...
Fork note: modified installation instructions based on steps already taken in this fork
This is a pre-patched repository 
1. Follow the steps in the TrinityCore (3.3.5) section of this readme
2. Merge worldserver.conf.dist into your worldserver.conf file (NPCBot mod settings)  
3. Apply SQL files from `/TrinityCore/sql/Bots/` to your DB (files starting with `characters_` and `world_` go into your `characters` and `world` DB respectively):
```
