...
void CharacterDatabaseConnection::DoPrepareStatements()
{
    ...
    // fork start - zzTransmogCompatibility - remove database qualifer from table name in case database name is different (e.g. world_test)
    // used to help show transmog on NPCBot
    PrepareStatement(CHAR_SEL_NPCBOT_TRANSMOG_BY_GUID, "SELECT ct.FakeEntry FROM item_instance ii INNER JOIN custom_transmogrification ct ON ii.guid = ct.GUID WHERE ct.GUID = ?", CONNECTION_SYNCH);
    // Player_Transmogrify.OnSave deletes then re-adds all transmogged items to get rid of stranded entries.  This makes it factor in NPCBots inventory and gear storage
    PrepareStatement(CHAR_SEL_NPCBOT_ITEM_GUID, "SELECT equipMhEx AS item_guid FROM characters_npcbot WHERE owner = ? and equipMhEx <> 0 UNION SELECT equipOhEx AS item_guid FROM characters_npcbot WHERE owner = ? and equipOhEx <> 0 UNION SELECT equipRhEx AS item_guid FROM characters_npcbot WHERE owner = ? and equipRhEx <> 0 UNION SELECT equipHead AS item_guid FROM characters_npcbot WHERE owner = ? and equipHead <> 0 UNION SELECT equipShoulders AS item_guid FROM characters_npcbot WHERE owner = ? and equipShoulders <> 0 UNION SELECT equipChest AS item_guid FROM characters_npcbot WHERE owner = ? and equipChest <> 0 UNION SELECT equipWaist AS item_guid FROM characters_npcbot WHERE owner = ? and equipWaist <> 0 UNION SELECT equipLegs AS item_guid FROM characters_npcbot WHERE owner = ? and equipLegs <> 0 UNION SELECT equipFeet AS item_guid FROM characters_npcbot WHERE owner = ? and equipFeet <> 0 UNION SELECT equipWrist AS item_guid FROM characters_npcbot WHERE owner = ? and equipWrist <> 0 UNION SELECT equipHands AS item_guid FROM characters_npcbot WHERE owner = ? and equipHands <> 0 UNION SELECT equipBack AS item_guid FROM characters_npcbot WHERE owner = ? and equipBack <> 0 UNION SELECT equipBody AS item_guid FROM characters_npcbot WHERE owner = ? and equipBody <> 0 UNION SELECT item_guid FROM characters_npcbot_gear_storage WHERE guid = ? and item_guid <>0;", CONNECTION_SYNCH);
    ...
    PrepareStatement(CHAR_DEL_NPCBOT_TRANSMOG_SLOT, "DELETE FROM characters_npcbot_transmog WHERE entry = ? and slot = ?", CONNECTION_ASYNC); // fork - zzTransmogCompatibility
