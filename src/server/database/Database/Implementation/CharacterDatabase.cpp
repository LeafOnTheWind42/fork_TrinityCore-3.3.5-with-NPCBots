...
void CharacterDatabaseConnection::DoPrepareStatements()
{
    ...
    PrepareStatement(CHAR_SEL_NPCBOT_TRANSMOG_BY_GUID, "SELECT it.displayid FROM characters.item_instance ii INNER JOIN characters.custom_transmogrification ct ON ii.guid = ct.GUID INNER JOIN world.item_template it ON ct.FakeEntry = it.entry WHERE ct.GUID = ?", CONNECTION_SYNCH); // TransmogDisplayVendor/NPCBot compatibility - SQL used to help show transmog on NPCBot
